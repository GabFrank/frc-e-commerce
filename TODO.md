...
- [ ] **Variant row admin con scroll horizontal en mobile** — `apps/web/components/admin/products/VariantForm.tsx:391` usa `min-w-[640px]` para no aplastar la fila, pero en mobile fuerza scroll horizontal y los botones de acción (editar, archivar, imágenes) quedan cortados a la derecha. **Fix correcto**: cuando viewport < md, renderizar como card stackeable (chips color+size arriba, nombre+SKU, precio/stock en línea, badge+acciones abajo); mantener fila horizontal en md+. Detectado al revisar capturas mobile el 2026-05-14 después del pase responsive.
- [ ] **Image upload "failed to fetch" en producción multi-tenant** — al subir imagen en `/admin/productos/[id]` desde subdominio del tenant (ej. `sublime-sleepwear.frc-ecommerce.com`), `ImageUploader` muestra "failed to fetch" en rojo. Posibles causas a investigar: (1) la cookie de sesión no acompaña al POST presigned, (2) R2 CORS rechaza el subdominio, (3) el cliente apunta a `NEXT_PUBLIC_APP_URL` fijo en vez de `window.location.origin`. Reproducir desde móvil en subdominio en prod. NO es bug del refactor responsive — preexistente.
