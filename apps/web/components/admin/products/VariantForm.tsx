...
function VariantRow({
  ...
  const colorChip = v.color ? (
    <span className="rounded-md border bg-secondary/40 px-2 py-1 text-center text-xs font-medium truncate min-w-[3.5rem]">
      {v.color}
    </span>
  ) : (
  ...
  const actions = (
    <div className="flex gap-1">
      <Button type="button" size="sm" variant="ghost" onClick={onEdit} title="Editar variante">
      ...
      >
        ...
        <span className="sr-only">{v.active ? 'Archivar' : 'Reactivar'}</span>
      </Button>
    </div>
  );
  const opacityCls = v.active ? '' : 'opacity-60';

  return (
    <>
      {/* Desktop — fila horizontal */}
      <div
        className={`hidden grid-cols-[auto_auto_1fr_auto_auto_auto_auto] items-center gap-3 px-3 py-2.5 text-sm transition-opacity md:grid ${opacityCls}`}
      >
        {colorChip}
        {sizeChip}
      ...
      {/* Mobile — card stackeado */}
      <div className={`flex flex-col gap-2 px-3 py-3 text-sm transition-opacity md:hidden ${opacityCls}`}>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <span className="text-muted-foreground">
            <span className="font-mono text-foreground">{formatAmount(v.price, productCurrency)}</span> · stock{' '}
            <span className="font-mono text-foreground">{v.stock}</span>
          </span>
        </div>
        <div className="border-t pt-2">{actions}</div>
      </div>
    </>
  );
}
